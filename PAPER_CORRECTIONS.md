...
论文图指北针箭头及N采用白色并加细黑描边，按当地真北方向绘制。坐标轴将下边框和左边框刻度位置转换为WGS84经纬度，以度分秒标注，例如120°56′12.3″E、30°42′50.6″N；秒保留一位小数，舍入时正确处理进位。图像及预测GeoTIFF保留原UTM投影网格，比例尺仍以米表示；这一显示调整不重投影或重采样预测数据。更新后使用上述`--figures-only`命令即可重绘全部三种图。

纬度刻度文字旋转90°纵向显示，预测图及叠加图的色柱竖直放在地图右侧，“预测扬花率 / %”横向放在色柱下方。比例尺线采用白色并加细黑描边，保持浅色背景下的可读性。这些版式修改仅影响PNG/PDF输出。
